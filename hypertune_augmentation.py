    ...
    with strategy.scope():
        model = keras.Sequential(
            [
                ...
                layers.RandomZoom(height_factor=zoomFactor, fill_mode='reflect', interpolation='bilinear'),
                layers.RandomTranslation(height_factor=0.0, width_factor=transFactor, fill_mode='reflect',
                                         interpolation='bilinear'),
                layers.RandomFlip(mode="horizontal_and_vertical"),
                layers.RandomRotation(factor=rotationFactor, fill_mode='reflect', interpolation='bilinear'),
                layers.RandomBrightness(factor=brightnessFactor),
                layers.RandomContrast(factor=contrastFactor),
                layers.GaussianNoise(stddev=noiseFactor),
...
print(f"""
The hyperparameter search is complete. The optimal value for zoom_factor is {best_hps.get('zoom_factor')}. The 
optimal value for translation_factor is {best_hps.get('translation_factor')}. The optimal value for rotation_factor is
{best_hps.get('rotation_factor')}. The optimal value for brightness_factor is {best_hps.get('brightness_factor')}.
The optimal value for contrast_factor is {best_hps.get('contrast_factor')}. The optimal value for noise_factor is
{best_hps.get('noise_factor')}
""")
